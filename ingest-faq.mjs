// ingest-faq.mjs
// FAQ 청크를 임베딩해서 Supabase knowledge_base 에 넣는 1회성 스크립트.
...
const CHUNKS_FILE = join(__dirname, 'faq_chunks.json');
...
/* ── 재실행 안전장치: 기존 source='qa' 전부 삭제 ── */
async function deleteExistingQa() {
  const res = await fetch(KB_ENDPOINT + '?source=eq.qa', {
    method: 'DELETE',
    headers: sbHeaders,
  });
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    throw new Error('기존 qa 삭제 실패 (HTTP ' + res.status + '): ' + body.slice(0, 500));
  }
}
...
async function main() {
  ...
  try {
    ...
    fail('faq_chunks.json 읽기/파싱 실패: ' + err.message);
  }
  if (!Array.isArray(chunks) || chunks.length === 0) {
    fail('faq_chunks.json 에 유효한 청크가 없습니다.');
  }
  console.log('청크 ' + chunks.length + '건 로드됨');

  // 기존 qa 삭제
  console.log('기존 source=qa 행 삭제 중...');
  await deleteExistingQa();
  console.log('기존 qa 삭제 완료');
