(function () {
    ...
    var DEEPLINKS = {
        kakao: {
            ...
            other:   'https://t.kakao.com/launch'
        },
        uber: {
...
<div id="taxi-btns-wrap">\
    <button class="taxi-btn-item taxi-btn-app" onclick="launchApp(\'kakao\')" aria-label="카카오T 택시">\
        <div class="taxi-btn-icon">' + kakaoSvg + '</div>\
        <span class="taxi-btn-label">Taxi</span>\
    </button>\
    <button class="taxi-btn-item taxi-btn-app" onclick="launchApp(\'uber\')" aria-label="우버">\
        <div class="taxi-btn-icon">' + uberSvg + '</div>\
        <span class="taxi-btn-label">Taxi</span>\
    </button>\
    <div id="food-delivery-group">\
        <div id="food-delivery-icons">\
            <button class="taxi-btn-item food-icon taxi-btn-app" onclick="launchApp(\'baemin\')" aria-label="배달의민족">\
                <div class="taxi-btn-icon"><img src="/images/baemin.svg" alt="배민" style="width:100%;height:100%;object-fit:contain;display:block;"></div>\
            </button>\
            <button id="taxi-btn-coupang" class="taxi-btn-item food-icon taxi-btn-app" onclick="launchApp(\'coupang\')" aria-label="쿠팡이츠">\
...
<div id="mobile-bottom-bar">\
    <button class="mbb-item" onclick="launchApp(\'kakao\')" aria-label="카카오T 택시">\
        <div class="mbb-icon">' + kakaoSvg + '</div>\
        <span class="mbb-label">Taxi</span>\
    </button>\
    <button class="mbb-item" onclick="launchApp(\'uber\')" aria-label="우버">\
        <div class="mbb-icon">' + uberSvg + '</div>\
        <span class="mbb-label">Taxi</span>\
    </button>\
    <button class="mbb-item" onclick="launchApp(\'baemin\')" aria-label="배달의민족">\
    ...
    <button class="mbb-item mbb-coupang" onclick="launchApp(\'coupang\')" aria-label="쿠팡이츠">\
...
.mbb-label {\
    ...
    // 딥링크 실행 — OS 감지 + fallback
    window.launchApp = function(appId) {
        var info = DEEPLINKS[appId];
        if (!info) return;

        // 배민·쿠팡이츠: 공식 유니버설 링크 (앱/웹/스토어 자체 처리)
        if (info.url) {
            window.open(info.url, '_blank');
            return;
        }

        // 카카오택시: 안드로이드 intent / 그 외 t.kakao.com
        if (appId === 'kakao') {
            if (isAndroid()) {
                window.location.href = info.android;
            } else {
                window.open(info.other, '_blank');
            }
            return;
        }

        // 우버: 커스텀 스킴 → 1.5s 내 앱 전환 없으면 웹 fallback
        if (appId === 'uber') {
            var appOpened = false;
            var onHide = function() {
            ...
            setTimeout(function() {
                ...
                if (!appOpened) { window.open(info.web, '_blank'); }
            }, 1500);
        }
    };
})();
